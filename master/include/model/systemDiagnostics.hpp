...
struct FailureDetection
{
    ...
    Metro dc_voltage_drop_timestamp_{DC_VOLTAGE_TIMEOUT}; // timer to check if dc voltage drops below threshold for more than 150ms
    Metro dc_voltage_hold_timestamp_{DC_VOLTAGE_HOLD};    // timer for ts on, only after enough voltage for 1 sec
